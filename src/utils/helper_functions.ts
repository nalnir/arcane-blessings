...
export const randomChance = (chancePercentage: ChancePercentage) => {
    const randomValue = Math.random() * 100; // Generates a random number between 0 and 99
  return randomValue < chancePercentage;
}

export const damage = (cardToDamage: OriginalCard, attackingCard: OriginalCard): OriginalCard => {
...
export const shouldDiscard = (cardToDiscard: OriginalCard) => {
...
export const discard = (cardToDiscard: OriginalCard, discardFrom: OriginalCard[], discardTo: OriginalCard[]): {discardFrom: OriginalCard[], discardTo: OriginalCard[]} => {
